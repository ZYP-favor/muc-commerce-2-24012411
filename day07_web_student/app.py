...
@login_required
def segments():
    # Load the segment analysis CSV using BASE_DIR so paths are robust when running the app
    csv_path = BASE_DIR / "data" / "segment_analysis.csv"
    ...
    # Ensure churn_rate is shown as percentage string like '53.54%'
    if "churn_rate" in df.columns:
        df["churn_rate"] = (df["churn_rate"].astype(float) * 100).round(2).astype(str) + "%"
    
    seg_list = df.to_dict("records")
    return render_template("segments.html", segments=seg_list)
...
if __name__ == "__main__":
    app.run(debug=False, port=526930)
